...
class InvoiceMessages::ReconcilePendingDeliveriesJobTest < ActiveJob::TestCase
  setup do
  ...
  test "marks stale outbound pending messages failed without enqueuing another delivery" do
    travel_to Time.zone.local(2026, 7, 22, 12) do
      ...
      assert_no_enqueued_jobs do
        InvoiceMessages::ReconcilePendingDeliveriesJob.perform_now
      end

      stale_message.reload
      assert_predicate stale_message, :status_failed?
      assert_equal InvoiceMessages::ReconcilePendingDeliveriesJob::FAILURE_REASON,
