...
class OutboundEmailConnection::DeliveryAvailabilityTest < ActiveSupport::TestCase
  setup do
  ...
  test "reports a missing outbound connection" do
    account = Account.create!(name: "Account Without Outbound Email")

    result = availability(account:)

    assert_not_predicate result, :ready?
    assert_nil result.connection
    assert_equal "missing_outbound_email_connection", result.reason
  end

  test "reports an inactive outbound connection as missing" do
    @account.outbound_email_connection
    OutboundEmailConnection.where(id: @connection.id).update_all(status: :disconnected)
  ...
  test "rejects a connection belonging to another account" do
    ...
    other_connection = other_account.create_outbound_email_connection!(
      provider: :gmail,
    ...
    @account.stubs(:outbound_email_connection).returns(other_connection)
